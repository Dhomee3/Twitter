...
async function getFollowingCount(_accountId: number): Promise<number> {
  return Follow.countDocuments({ followerId: _accountId });
}
...
async function handleModal(i: ModalSubmitInteraction): Promise<any> {
  ...
  if (customId === "modal_tweet") {
    ...
    if (!settings?.tweetChannelId) return i.editReply({ embeds: [err("لم يتم الإعداد", "اطلب من الأدمن تحديد روم التغريدات من بانل الأدمن.")] });
  ...
  if (customId.startsWith("modal_comment")) {
    ...
    if (!tweet) return i.editReply({ embeds: [err("تغريدة غير موجودة", `لا توجد تغريدة بالمعرف #${tweetId}.`)] });
    await updateAvatar(account.id, i.user.displayAvatarURL({ size: 128 }));
    account.avatarUrl = i.user.displayAvatarURL({ size: 128 });
    const existingComment = await Comment.findOne({ authorId: account.id, tweetId });
    if (existingComment) return i.editReply({ embeds: [err("خطأ", "لا يمكنك التعليق على التغريدة أكثر من تعليق واحد.")] });
    const comment = await createComment(tweetId, account.id, content);
    const settings = await getSettings(i.guildId!);
    if (settings?.tweetChannelId) {
      try {
        const ch = await i.client.channels.fetch(settings.tweetChannelId) as TextChannel;
        await ch.send({ embeds: [commentEmbed(comment, account, tweetId)] });
        if (tweet.messageId && tweet.channelId) {
          ...
          if (updated && author) {
            ...
            const msg = await tweetCh.messages.fetch(tweet.messageId);
            await msg.edit({ embeds: [tweetEmbed(updated, author, f, c)], components: [tweetButtons(tweetId, author.id, updated.likeCount ?? 0)] });
          }
        }
      } catch { /* ignore */ }
    }
    return i.editReply({ embeds: [ok("تم إرسال التعليق! 💬", "تم إضافة تعليقك بنجاح.")] });
  }
...
async function startBot(): Promise<void> {
  ...
  if (clientId) {
    ...
    console.warn("DISCORD_CLIENT_ID not set — slash commands will not be registered.");
  }
...
async function main() {
  ...
  // HTTP health check — required for Render Web Service
